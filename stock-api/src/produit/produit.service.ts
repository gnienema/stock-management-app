...
export class ProduitService {
  ...
  async create(createProduitDto: CreateProduitDto) {
    ...
    if (!reference || reference.trim() === '') {
      ...
      const lastIdResult = await this.db.query(`SELECT id FROM produit ORDER BY id DESC LIMIT 1`);
      const lastId = lastIdResult.rowCount > 0 ? lastIdResult.rows[0].id : 0;
    ...
    try {
      ...
      console.log('Succès SQL:', result.rows[0]);
...
// ... (Gardez les autres méthodes: findOne, update, remove, etc.)
  ...
  async findOne(id: number) {
    ...
    if (result.rowCount === 0) throw new NotFoundException(`Produit introuvable`);
    return result.rows[0];
  }
  ...
  async remove(id: number) {
    await this.db.query(`DELETE FROM produit WHERE id = $1`, [id]);
